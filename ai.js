...
function AITest(p) {
	...
	this.buyProperty = function(index) {
		console.log(p.name + ": buyProperty");
		var s = square[index];
	...
	this.acceptTrade = function(tradeObj) {
		console.log(p.name + ": acceptTrade");
		...
		// var proposedMoney = -tradeValue + money;

		if (tradeValue >= 25) {
			return true;
		} else if (tradeValue >= -50 && (initiator.human || tradeValue !== -25) && initiator.money > proposedMoney) {
	...
	this.beforeTurn = function() {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (s.owner === p.index && s.groupNumber >= 3) {
				...
				if (p.money > leastHouseProperty.houseprice + 100 && s.house < 5) {
					buyHouse(leastHouseProperty.index);
					console.log(p.name + " put a house on " + square[leastHouseProperty.index].name + " on turn " + turnCount + ".");
				}
			}
		}
	...
	this.onLand = function() {
		...
		if (utilityForRailroadFlag && requestedRailroad && offeredUtility) {
